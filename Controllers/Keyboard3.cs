...
namespace Sprint2
{
    class Keyboard3 : IController
    {
        private Dictionary<Keys, ICommand> keyCommands;
        private List<Keys> previouslyPressed;

        public Keyboard3(MarioGame game, Koopa koopa)
        {
            keyCommands = new Dictionary<Keys, ICommand>();
            keyCommands.Add(Keys.Q, new QuitGame(game));

            keyCommands.Add(Keys.A, new KoopaChangeDirectionCommand(koopa));
            keyCommands.Add(Keys.D, new KoopaChangeDirectionCommand(koopa));
            keyCommands.Add(Keys.S, new KoopaStompedCommand(koopa));
            keyCommands.Add(Keys.W, new KoopaFlipCommand(koopa));
